...
decipher_gpsm <- read_decipher_gpsm(gpsm_folder_dir)
...
pGlyco3_gpsm; decipher_gpsm
```


```{r}
decipher_trait_se <- build_trait_se(
  decipher_gpsm,
  from = "decipher", 
  motifs = NULL,
  level = "protein",
...
```{r}
decipher_res <- analyze_trait_changes(
  decipher_trait_se,
  group_col = "Diagnosis",
...
i <- sample(nrow(decipher_res), 1)
trait <- decipher_res[i,1]
level <- decipher_res[i,2]
