...
#' glycan topology (branching, residue types, connectivity) during parsing
#' and debugging.
#'
#' @param g
#' An `igraph` object representing a glycan tree.
#' Nodes must contain:
#' * `V(g)$type`  — residue type (e.g., "H", "N", "A")
#' * `V(g)$color` — residue category color
#'
#' @details
#' The tree is rendered using:
#'
#' * a root-directed tree layout (`layout_as_tree`)
#' * residue types as vertex labels
#' * residue-type–specific coloring determined by `build_glycan_igraph()`
#'
#' This function produces a base R plot and returns `NULL` invisibly.
#'
#' @return
#' Invisibly returns `NULL`. A glycan topology plot is drawn as a side effect.
#'
#' @examples
#' \dontrun{
#' g <- build_glycan_igraph(wurcs_to_tree(""WURCS=2.0/4,9,8/
#' [a2122h-1b_1-5_2*NCC/3=O][a1122h-1b_1-5][a1122h-1a_1-5][a4344h-1a_1-?]/
#' 1-1-2-3-3-3-4-4-3/a4-b1_b4-c1_c3-d1_c6-i1_d2-e1_e2-f1_f3-g1_g2-h1"))
#' plot_glycan_tree(g)
#' }
#'
#' @import igraph
...
#' This function extracts PSM-level values from a trait matrix stored in a
#' `SummarizedExperiment`, subsets them by group, removes missing values,
#' and visualizes the resulting distribution.
#'
#' @param trait_se
#' A `SummarizedExperiment` object generated by [`build_trait_se()`],
#' containing one assay per glycan trait.
#'
#' @param group_col
...
#'
#' @param feature
#' Row identifier within the trait matrix (site, peptide, or protein label).
#' Must match a row name in `assays(trait_se)[[trait_name]]`.
#'
#' @details
#' Returned plots include:
#'
#' * **Histogram** — distribution of PSM-level trait intensities per group
#' * **Boxplot + jitter** — comparison of group-level distributions
#'
#' Only non-`NA` PSM values are used.
#' Colors are automatically assigned to match the order of `group_levels`.
#'
#' @return
...
plot_trait_distribution <- function(trait_se,
  ...
  p_freq <-
    ...
    labs(x = paste(trait, "Count at", feature), y = "PSM count") +
  ...
  p_box <- ggplot(df_plot, aes(x = Group, y = Abundance, fill = Group)) +
    ...
    scale_fill_manual(values = setNames(c("#e31a1c", "#1f78b4"), group_levels)) +
    labs(x = NULL, y = paste(trait, "Count at", feature)) +
    theme_classic(base_size = 14) +
    theme(
      axis.text.x = element_text(size = 14),
      axis.text.y = element_text(size = 12),
